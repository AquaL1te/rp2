...
As a result, if transparency is implemented the user might not even be aware of an \gls{ndn}. The user only specifies a \gls{pid} as input for the client, without specifying the link of the web resolver that resolves the PID. The user gets redirected automatically by the client to either the \gls{pid} server or \gls{ndn}. No further user input is needed as everything is taken care of by the gateway, which will also be discussed in section \ref{gw}.
