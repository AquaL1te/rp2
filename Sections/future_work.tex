...
% However, some issues are identified with NDN such as underlay performance issues when using UDP and the effect of long names - mention UDT as future work for integration with NDN since UDP in NDN does not utilize congestion and flow control mechanisms.

% Research clouds are in general a federated cloud, where each federation is responsible for its own budget and infrastructure. However, our solution assumes central control over the NDN. Our solution could be approached in several ways. The NDN could be deployed as an internal data-sharing platform per infrastructure, thus maintaining the federated model. Or, it could be deployed as a third party data-sharing platform, where one can deploy and operate for multiple infrastructures.
