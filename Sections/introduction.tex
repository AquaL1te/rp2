...
SeaDataNet connects together more than 100 data centres from eight European marine institutes in different geographical domains, aiming at preserving and making re-useable marine observations ranging from ocean physics to chemistry and biology. 
...
Their proposed model currently only supports one PID provider also referred to as a PID type. Interoperability between different PID types within NDN has not been realized yet.
...
Based on related work and the research question we defined the following scope for our research. PID X and PID Y will be made interoperable with NDN. This goal entails having this and that end result.
