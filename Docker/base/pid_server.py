...
# Download file and insert it in NDN (ndnputchunks): has to be done, but before this, already send the PID link to client so it does not have to wait to download the PID file and insert it in NDN.
            urllib.request.urlretrieve('http://hdl.handle.net/' + pid, filename)

# Publish it in NDN, write a different script for this and run in background?
...
# Download file and insert it in NDN (ndnputchunks): has to be done, but before this, already send the PID link to client so it does not have to wait to donwload the PID file and insert it in NDN.
